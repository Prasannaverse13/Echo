...
| Requirement | Echo |
|---|---|
| Gemini 3.5+ (Flash) | `gemini-3.5-flash` via `@google/genai` (preferred, satisfies "3.5 or newer"); falls back to `gemini-2.5-flash` via `@google-cloud/vertexai` when the project's AI Studio credits are exhausted or the model isn't provisioned |
